...
def event_metric_MORT24h(event, inference_output, mode, model_name):
    ...
    usecol = [icu_stay,'Time_since_ICU_admission', 'death', 'prediction_label']
    ...
    total_event = event_set.death.value_counts()['event']
    captured_event = 0
    ...
    for stayid in total_set[icu_stay].unique():
        ...
        if any(interest['death']=='event'):
            
            total_event += 1
            event_time = interest['Time_since_ICU_admission'].iloc[-1]
            time_window = event_time - 24
            capture_before_8h = interest[(interest['Time_since_ICU_admission'] > time_window) & (interest['Time_since_ICU_admission'] < event_time)]
...
def MORT24h_AUPRC(event, inference_output, mode, model_name):
    ...
    # 예측 확률과 실제 레이블이 필요
    y_true = evaluation_interest.death # 실제 레이블
    y_scores =evaluation_interest.prediction_prob # 예측 확률

    precisions, recalls, threshold_pr = precision_recall_curve(y_true, y_scores)
    new_recalls = []
    ...
    for threshold in thresholds:
        ...
        recall = event_metric_MORT24h(event, evaluation_interest, mode, model_name)
        new_recalls.append(recall)
    
    # auprc = auc(np.array(new_recalls), precisions[:-1])
    # Map new_recalls to closest threshold in precision_recall_curve
    closest_idx = np.searchsorted(threshold_pr, thresholds) - 1
    closest_precisions = precisions[closest_idx]

    # Calculate AUC
    auprc = auc(np.array(new_recalls), closest_precisions)
    return auprc
