...
def split_X_Y_for_VIZ(df, mode):
    ...
    if mode == 'mimic':
        X = df.drop(['subject_id', 'stay_id', 'hadm_id','Annotation', 'Case', 'Shock_next_8h', 'INDEX'], axis = 1)
        y = df['Case'].values
        output = df[['stay_id', 'Time_since_ICU_admission', 'vasoactive/inotropic', 'Fluids(ml)', 'Annotation', 'progress', 'Case', 'INDEX', 'Lactate', 'MAP']].copy().reset_index(drop=True)
        output['Case'] = output['Case'].astype(int)
        
    else:
        X = df.drop(['uniquepid', 'patientunitstayid','Annotation', 'Case', 'Shock_next_8h', 'INDEX'], axis = 1)
        y = df['Case'].values
        output = df[['patientunitstayid', 'Time_since_ICU_admission', 'vasoactive/inotropic', 'Fluids(ml)', 'Annotation','progress','Case', 'INDEX', 'Lactate', 'MAP']].copy().reset_index(drop=True)
        output['Case'] = output['Case'].astype(int)
