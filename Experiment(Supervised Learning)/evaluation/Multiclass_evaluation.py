...
def create_analysis(eventset,X_train, y_train, X_valid, valid_output):
    ...
    for model_name in model_names:
        ...
        model, valid_output = classifier_ML.classifier(model_name, X_train, y_train, X_valid, valid_output)
...
def MULT_evaluation(inference_output, event, model_name, mode):
    ...
    if (inference_output.Case.nunique()==4):
        ...
        data = {'Model':[np.nan], 'Accuracy': [np.nan],'Case 1 Score': [np.nan]
                ,'Case 2 Score': [np.nan], 'Case 3 Score': [np.nan],'Case 3 Score': [np.nan], 'Case 4 Score': [np.nan]}
