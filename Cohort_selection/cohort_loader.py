...
class TrainingDataset(Dataset):
    ...
    def __prepare_data__(self):
        ...
        if self.data_type == 'mimic':
            ...
            if self.mode == "train":
                X_num = df_train[self.num_features].drop(['Case', 'stay_id', 'subject_id','hadm_id', 'ethnicity','Annotation'], axis = 1)
                ...
                X_num_standard = df_train[self.num_features].drop(['Case', 'stay_id', 'subject_id','hadm_id','ethnicity', 'Annotation'], axis = 1)
                scaler.fit(X_num_standard)
                
                X_num = df_valid[self.num_features].drop(['Case', 'stay_id', 'subject_id','hadm_id', 'ethnicity', 'Annotation'], axis = 1)
        ...
        # if dataset is eicu
            ...
            mimic = pd.read_csv('/Users/DAHS/Desktop/ECP_CONT/ECP_SCL/Case Labeling/eicu_analysis.csv.gz', compression='gzip')
...
class EventDataset(Dataset):
    def __init__(self,data_path,data_type,seed):
        self.data_path = data_path
        self.data_type = data_type # eicu or mimic
        self.target = 'Case'
        self.seed = seed
